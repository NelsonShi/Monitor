...
function Sales ({ data }) {
  return (
    <div className={styles.sales}>
      <div className={styles.title}>Hourly Error</div>
      <ResponsiveContainer minHeight={250}>
        <LineChart data={data}>
          ...
          />
          <XAxis dataKey="hour" axisLine={{ stroke: '#e5e5e5', strokeWidth: 1 }} tickLine={false} />
          ...
          />
          <Line type="monotone" dataKey="errorCount" stroke='#f8c82e' strokeWidth={3} dot={{ fill: '#f8c82e' }} activeDot={{ r: 5, strokeWidth: 0 }} />
        </LineChart>
      </ResponsiveContainer>
    </div>
  )
